...
1. **非官方项目**：本项目为个人开源项目，与 OpenAI、Google、Anthropic 等公司及其产品无任何关联，不代表其官方立场或技术支持。
2. **使用风险自负**：本项目通过模拟浏览器行为与第三方 AI 平台交互，可能违反相关平台的服务条款。由此导致的账号封禁、数据丢失等后果由使用者自行承担，项目作者不承担任何责任。
3. **仅供学习研究**：本项目仅供学习和技术研究使用，请勿用于任何商业用途或非法用途。
4. **不保证可用性**：由于依赖第三方台的非公开接口，本项目不保证服务的稳定性、连续性和准确性。第三方平台接口变更可能导致功能失效，作者不承诺及时修复。
5. **隐私与安全**：本项目会在本地 Chrome profile 中保存登录凭证，请确保在可信环境中使用。作者不会收集或上传任何用户数据，但使用者应自行评估隐私风险。
6. **法律合规**：使用者应遵守所在国家/地区的法律法规，因违规使用本项目而产生的任何法律责任由使用者自行承担。
